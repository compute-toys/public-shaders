...
#define CurrentTime (time.elapsed * 0.03)
...
fn force(v: float3, t: float) -> float3 {
    let sc = custom.P2*(rot(3.215*CurrentTime)+0.15*rot(15.515*CurrentTime).yx);
    let sc2 = 7.0*rot(2.0*CurrentTime) + 5.0*rot(0.25*3.14519*2.0*CurrentTime);
    let sc3 = 5.0*rot(1.3547*CurrentTime) + 5.0*rot(2.7547*CurrentTime);
    let sc4 = custom.P3*rot(0.4*CurrentTime + t);
...
fn point_gen() {
    ...
    let sc = custom.P4*rot(0.5*CurrentTime);
