...
fn fun(@builtin(global_invocation_id) id3: vec3u)
{
    ...
    for(var i=0; i<ZL; i++)
    {
        ...
        u = vec3f(atan2(u.y,u.x),
                  atan2(u.z,length(u.xy)),
                  log(length(u)));    u *= vec3f(1,1,1)*tm;
        u = vec3f(cos(u.y)*cos(u.x),
                  cos(u.y)*sin(u.x),
                  sin(u.y))*exp(u.z);   u = u+custom.b;
