...
fn fun(@builtin(global_invocation_id) id3: uint3)
{
    ...
    for(var x=-rZ; x<=rZ; x+=1.f){
        ...
        var d2 = pO(float3(D2[i2+0u*rez],
                           D2[i2+1u*rez],
                           D2[i2+2u*rez]),
                           D2[i2+3u*rez]);
        var l2 = sqrt(l);
        var dv = d.v;//-d2.v;
        var vl = length(dv);  if(vl!=0.f){vl = 1.f/vl;}
        var f  = xyz/l2*(d.m-d2.m)/exp(l*.2f);
        var dt = dot(dv*vl,xyz)*custom.A*6.f;
        v += f*cos(dt);
        v += f*sin(dt);
    }}} d.v += v;
...
fn main_image(@builtin(global_invocation_id) id: uint3)
{
    ...
    var lig = float3(2);        //light created by camera going to voxels
    var rif = float3(0);        //reflected light by voxels going to camera
    for(var i=0u; i<x4; i++)    //ray will transverse simulationCube's voxels
    {
        ...
        var d = rD(fw+c31(uint3(p)));   //voxel value at "p"
        var n = lig*max(1.f-d.m*l,0.f);    //light after some energy absorved by voxel                          
        rif += (lig-n)*mix(d.v,float3(1),d.m);    //light emited by voxel
        lig  = n;
        p += ray*l*1.001f;                                 //make ray transverse one voxel
        var o = abs(p*x3d-1.f);
        if(o.x>=1.f||o.y>=1.f||o.z>=1.f){break;}    //ray got out of simulationCube
    }
    textureStore(screen, int2(id.xy), float4(rif*tF,0));
}
