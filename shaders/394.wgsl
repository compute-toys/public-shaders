...
fn comb(@builtin(global_invocation_id) id3: uint3)
{
    ...
    for(var x=-rZ; x<=rZ; x+=1.f){
        ...
        var l  = sqrt(l2);
        var xyl= xy/l;
        v1 += dot(d2.v-d.v,xyl                )/exp(l2*.3f); //divergence
        v2 += dot(d2.v-d.v,xyl.yx*float2(-1,1))/exp(l2*.3f); //rotation
    }}
    C[id] = float2(v1,v2);
}
...
fn fun(@builtin(global_invocation_id) id3: uint3)
{
    ...
    for(var x=-rZ; x<=rZ; x+=1.f){
        ...
        var l  = sqrt(l2);
        var xyl= xy/l;
        v += (float(c2.y>=0.f)*2.f-1.f)*xyl.yx*float2(-1,1)*c2.x/exp(l2*.3f);
    }}
    d.v += v*(custom.A-.5f)*.2f;
    //reset simulation
    if(time.frame==0u)          
    {
        var u = (p/float(rez)-.5f)*4.f;
...
fn main_image(@builtin(global_invocation_id) id3: uint3)
{
    ...
    //var c  = sin(d.v.x*3.f+float4(1,3,5,4))*.25f +
    //         sin(d.v.y*3.f+float4(1,3,2,4))*.25f +.5f;
