...
fn main_image(@builtin(global_invocation_id) id: vec3u) {
    ...
    cell_coord = cell_coord * rotate_2d(t);
    ...
    let s = u32(modulo(time.elapsed, 2.));
    sdf = select(op_onion(sdf, 8., 4.), op_lnion(sdf, 8., 4.), s == 0);
...
fn op_lnion(d: f32, l: f32, t: f32) -> f32 {
    return abs(max(d, modulo(d + l, 2. * l) - l)) - t;
}