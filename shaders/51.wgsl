...
fn Solve(i: uint) -> float3
{
    ...
    let f = dV(a1);
    let ft0 = dC(a0);
    let fx0 = dC(Load(i - 1).x);
    let fx2 = dC(Load(i + 1).x);
    var dA = 0.0;
    var dT = STEP_SIZE;
    var a2prev = a2;
...
{
    return float2(SCREEN_WIDTH * (float(i) + 0.5)/ LENGTH, Load(i).x * SCREEN_HEIGHT/30.0 + SCREEN_HEIGHT * 0.5);
}
...
fn MainImage(@builtin(global_invocation_id) id: uint3) 
{
    ...
    {
        let sdSeg = sdSegment(fragCoord, GetPoint(i), GetPoint(i+1));
        if(sdSeg < sdf)
        {
            sdf = sdSeg;
            am = Load(i).z;
