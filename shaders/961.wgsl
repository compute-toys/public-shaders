...
fn fun(@builtin(global_invocation_id) id3: vec3u)
{
    ...
    var p = vec4f(0);
    var v = vec4f(0);
    var tim =  f32(time.frame)*.00000000002f;
    for(var i=0; i<ZT; i++)
    {
        workgroupBarrier();
        ...
        for(var j=0; j<T; j++)
        {
            ...
            var n = cos((f32(id1)*f32(i*T+j))*tim)*1.f+1.f;
            v -= d*nrm*(n-l);
        }
        r = (r+T) % Z;
    }
    v = v*(1.f/f32(Z))*vec4f(1,1,1,0);
    //v -= max(0.f,length(p)-1.5f)*normalize(p);
    var p2 = p+v;//-ps/f32(Z);
    P[id1+Z*0] = p2.x;
    P[id1+Z*1] = p2.y;
    P[id1+Z*2] = p2.z;
