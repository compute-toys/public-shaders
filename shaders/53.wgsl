...
#define SIM_FRAME (ITERATIONS*time.frame+dispatch.id)
#define IN_BUF (SIM_FRAME%2)
#define OUT_BUF ((SIM_FRAME+1)%2)
...
fn LagrangianField(A2: Dual, A1: Dual, p: int2, dp: int2) -> Dual
{
    ...
    let ax0 = LoadA(A1, i - uint2(1, 0), i0);
    let ax2 = LoadA(A1, i + uint2(1, 0), i0);
    let ay0 = LoadA(A1, i - uint2(0, 1), i0);
    let ay2 = LoadA(A1, i + uint2(0, 1), i0);

    let a0x0 = LoadA0(i - uint2(1, 0));
    let a0x2 = LoadA0(i + uint2(1, 0));
    let a0y0 = LoadA0(i - uint2(0, 1));
    let a0y2 = LoadA0(i + uint2(0, 1));

    let a2x0 = LoadA2(A2, i - uint2(1, 0), i0);
    let a2x2 = LoadA2(A2, i + uint2(1, 0), i0);
    let a2y0 = LoadA2(A2, i - uint2(0, 1), i0);
    let a2y2 = LoadA2(A2, i + uint2(0, 1), i0);
...
{
    if(id.x == 0u || id.x == LENGTH - 1u || id.y == 0u || id.y == LENGTH - 1u)
    {
    ...
    if(time.frame == 60 && id.y > uint(LENGTH/2))
    {
        f*=0.0;
    }
