...
#define IN_BUF ((ITERATIONS*time.frame+dispatch.id)%2)
#define OUT_BUF ((ITERATIONS*time.frame+dispatch.id+1)%2)
...
fn Solve(i: uint2) -> float3
{
    ...
    let f = dV(a1);
    let ft0 = dC(a0);
    let fx0 = dC(Load(i - uint2(1, 0)).x);
    let fx2 = dC(Load(i + uint2(1, 0)).x);
    let fy0 = dC(Load(i - uint2(0, 1)).x);
    let fy2 = dC(Load(i + uint2(0, 1)).x);
