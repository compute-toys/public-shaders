...
fn sd_sphere(p: float3, r: float3) -> float {
    return length(p) - r.x;
}

fn sd_box(p: float3, b: float3) -> float {
    let q = abs(p) - b;
    return length(max(q, vec3(0))) + min(max(q.x, max(q.y, q.z)), 0) - 0.03;
}

fn sd_cylinder(p: float3, rh: float3) -> float {
    let d = abs(vec2(length(p.xz), p.y)) - rh.xy;
...
fn signed_distance(obj: SDFObject, pos: float3) -> float {
    ...
    switch (obj.shape) {
        case SHAPE_SPHERE:
            { return sd_sphere(p, scale); }
        case SHAPE_BOX:
            { return sd_box(p, scale); }
        case SHAPE_CYLINDER:
            { return sd_cylinder(p, scale); }
        default:
            { return sd_sphere(p, scale); }
    }
}
...
fn calc_normal(obj: SDFObject, pos: float3) -> float3 {
    let position = obj.transform.position;
    let rotation = obj.transform.rotation;
    let scale    = obj.transform.scale;
    ...
    switch (obj.shape) {
        case SHAPE_SPHERE: { 
            n = e.xyy*sd_sphere(p + e.xyy, scale) + 
                e.yyx*sd_sphere(p + e.yyx, scale) + 
                e.yxy*sd_sphere(p + e.yxy, scale) + 
                e.xxx*sd_sphere(p + e.xxx, scale);
        }
        case SHAPE_BOX: { 
            n = e.xyy*sd_box(p + e.xyy, scale) + 
                e.yyx*sd_box(p + e.yyx, scale) + 
                e.yxy*sd_box(p + e.yxy, scale) + 
                e.xxx*sd_box(p + e.xxx, scale);
        }
        case SHAPE_CYLINDER: { 
            n = e.xyy*sd_cylinder(p + e.xyy, scale) + 
                e.yyx*sd_cylinder(p + e.yyx, scale) + 
                e.yxy*sd_cylinder(p + e.yxy, scale) + 
                e.xxx*sd_cylinder(p + e.xxx, scale);
        }
        default: { 
            n = e.xyy*sd_sphere(p + e.xyy, scale) + 
                e.yyx*sd_sphere(p + e.yyx, scale) + 
...
fn sample(uv: float2) -> float4 {
    ...
    for (var i = 0; i < MAX_RAYTRACE; i++) {
        let roulette_prob = pow(bright, 0.01);
        ...
        for (var i = 0; i < MAX_RAYMARCH && t < MAX_DIS && !hit; i++) {
            ...
            let dis = abs(nearest_object(pos, &obj_index));
        ...
        let obj = world[obj_index];

        var normal = calc_normal(obj, pos);
        ...
        ray.color *= albedo / roulette_prob;
        ray.origin = pos + normal * ray_bias * sign(dot(ray.direction, normal));
        ray.color  *= obj.material.emission;
        let visible = brightness(ray.color);
        
        if (bright < visible || visible < VISIBILITY) { break; }
        bright = visible;
    }
