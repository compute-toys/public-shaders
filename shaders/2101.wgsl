...
fn getSwirl(p_in: vec2f, time_offset: f32) -> f32 {
    var p = p_in;
    let d = length(p);
    let a = (time.elapsed + time_offset) * 0.1 + 2.5 / (d + 0.2);
    let c = cos(a);
    let s = sin(a);
    ...
    let noise_val = (N(p, 3.0, 0.2, time.elapsed) * 0.6 + N(p, 9.0, 0.5, time.elapsed) * 0.4) * 0.5 + 0.5;
    return noise_val * smoothstep(0.1, 0.15, d);
}

fn getSwirl2(p_in: vec2f, time_offset: f32) -> f32 {
    var p = p_in;
    let d = length(p);
    let a = (time.elapsed + time_offset) * 0.2 + 1.5 / (d + 0.2);
    let c = cos(a);
    let s = d; 

    let scale = 1.0 - cos(a * 7.0 - (time.elapsed * 2.0 + time_offset)) * 0.03;
...
fn renderScene(uv: vec2f) -> vec3f {
    let distance_from_center = cos(time.elapsed) * 0.01;
    let center = normalize(vec2f(1.0, 0.25)) * distance_from_center;
    let p = uv - center;

    let red_swirl   = getSwirl(p, 0.0);
    let green_swirl = getSwirl(p, 10.0);
    let blue_swirl  = getSwirl2(p, 20.0);
