...
#define ote 77f        //outside fractal space
#define rml 33u         //raymarch loop
#define G 64f           //size of fractal
#define ryT  64u        //threads
#define ryT2 1024u        //threads
...
fn getCol(p2: vec2f) -> vec4f
{
    ...
    for(var i=0; i<4; i++)
    {
        p*=m; s*=.5f;
        p = abs(p)-G*1.1f;
        var c = cos(f32(i)*PI*2f/4f+time.elapsed+vec4f(1,2,3,4))
                *vec4f(.2,.2,.2,0)+vec4f(.8,.8,.8,.0);
        var to = toru(p, 2f, 4f*G/9f, 1f*G/9f)*s;
        var l = 1f-4f*abs(to);
            l = max(l,0f);
        a += l*c;
...
fn main_image(@builtin(global_invocation_id) id3: vec3u)
{
    ...
    atomicStore(&D[r+0u*sWH], 0);//c.x*21/22);
    atomicStore(&D[r+1u*sWH], 0);//c.y*21/22);
    atomicStore(&D[r+2u*sWH], 0);//c.z*21/22);
    var d = vec4f(c)/f32(fti)*custom.brigt*.003f;
