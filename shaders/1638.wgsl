...
#define FOV 1.0
#define AXIS_COUNT 3
#define GROUP_COUNT 32768 //SIZE*SIZE
#define ELEMENT_COUNT SIZE*SIZE*SIZE

#define DISPATCHES_PER_STEP 8 //2*AXIS_COUNT + 2
#dispatch_count mega_kernel 8 //DISPATCHES_PER_STEP * STEP_COUNT
...
fn blobIC(pos: vec3f, bpos:vec3f, vel:vec3f, rad: float) -> vec2f
{
    ...
    return 1.05 * density * expi(-dot(vel, dx));
}
...
fn real_space_update(index: u32)
{
    ...
    if(time.frame < 5) 
    {
        ...
        let posf = vec3f(id);
        var InitCond = blobIC(posf, vec3f(0.25, 0.4, 0.55)*SIZE, vec3f(-0.5, 0.0, 0.0), 70.0);
        InitCond += blobIC(posf, vec3f(0.75, 0.5, 0.49)*SIZE, vec3f(0.5, 0.0, 0.0), 70.0);
        sim[index] = InitCond;
    } 
    else
    {
...
fn rayMarch(ray: Ray, dither: float) -> vec4f {
    ...
    for (var i = 0u; i < 512u; i++) {
        ...
        if (d > ISO_VALUE) {
            ...
            let n1 = select(1.0, 1.2, inside);
            let n2 = select(1.2, 1.0, inside);
            let refDir = refract(cray.rd, n, n1 / n2);

            if(inside) {
                absorption += 0.05*vec3(0.4, 0.1, 0.05) * tmid;
            } 
