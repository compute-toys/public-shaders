#define SIZE 64
#define IT_NUM 5 //floor(log2(N))-1
#define WG_SIZE 32
...
#define FOV 1.0
#define AXIS_COUNT 3
#define GROUP_COUNT 2048 //SIZE*SIZE/2
#define ELEMENT_COUNT SIZE*SIZE*SIZE

#define DISPATCHES_PER_STEP 8 //2*AXIS_COUNT + 2
#define STEP_COUNT 1
#define TOTAL_DISPATCH_COUNT 8 //DISPATCHES_PER_STEP * STEP_COUNT
...
fn real_space_update(index: u32)
{
    ...
    if(time.frame < 5) 
    {
        ...
        var InitCond = blobIC(posf, vec3f(0.5, 0.5, 0.5)*SIZE, vec3f(0.0, 0.0, 0.0), 20.0);
        sim[index] = InitCond;
    } 
    else
    {
        ...
        if(CUR_INDEX%2000 == 0)
        {
           field += blobIC(posf, vec3f(0.5, 0.4, 0.5)*SIZE, vec3f(0.0, 0.0, 0.0), 12.0);
        }
...
fn fresnel(h: vec3f, n: vec3f, n1: float, n2: float) -> float {
    let f0 = 0.05;
    return f0 + (1.0 - f0) * pow(1.0 - dot(h, n), 5.0);
}
