...
//to make all pair of points distant by f(x,y,l)
//x and y is the id of each point, l = distance between points
//f(x,y,l) = abs(x-y)*(cos(l*constA+constB)*.5+.5)
...
fn fun(@builtin(global_invocation_id) id3: vec3u)
{
    ...
    for(var i=0; i<ZT; i++)
    {
        ...
        for(var j=0; j<T; j++)
        {
            ...
            var n2 = f32(i*T+j);
            var z1 = f32(Z-1);
            var n = abs(n1-n2)*(cos(l*22.f*custom.A+custom.B*7.f)*.5f+.5f)*.0003f;
            v -= d*nrm*(n-l);
            v2+= pow(n-l,2.f);
        }
        r = (r+T) % Z;
    ...
    var col = pow(cos(f32(id1)/f32(Z)*4.f+5.f+vec4f(1,2,3,4))*.2f+.2f,vec4f(2))*f32(Z);
