...
fn buddhabrot(uv: vec2<f32>) -> vec4<f32> {
    var ro: vec3<f32> = vec3<f32>(0., 0., 0.);
    //var rd: vec3<f32> = vec3<f32>(rand4().xy - 0.5, -1.);
    //var rd: vec3<f32> = udir(rand4().xy);
    ...
    for(var i = 0; i < 64; i++) {
        ...
        //var d = min(length(x-vec3f(0.,0.,-1.))-.1,2.+x.z);
        var d = min(map((x-vec3(0.,0.,1.5)).xyz),1.-x.z);

        if(d < .0001) {
            //RasterizePoint(x, vec3f(1.,1.,1.)/(custom.Samples*MaxSamples + 1.0));
            let quantiz = exp2(10.*custom.quantization);
            t = floor((quantiz*t)+10.*custom.Uncertainty*(rand4().x-.5))/quantiz;
            // Uncertainty: fuzziness of the simulated LiDAR range estimate
            RasterizePoint(ro+(t*rd), (vec3f(1.,1.,1.)/(t*t))/(custom.Samples*MaxSamples + 1.0));
            break;
        }
...
#workgroup_count Rasterize 16 16 1
@compute @workgroup_size(16, 16)
fn Rasterize(@builtin(global_invocation_id) id: uint3) 
{
    ...
    RasterizePoint(vec3f(0.000,0.000,0.000),vec3f(0.010,0.010,0.010));
    RasterizePoint(vec3f(0.010,0.000,0.000),vec3f(0.010,0.000,0.000));
    RasterizePoint(vec3f(0.000,0.010,0.000),vec3f(0.000,0.010,0.000));
    RasterizePoint(vec3f(0.000,0.000,0.010),vec3f(0.000,0.000,0.010));

    for(var i: i32 = 0; i < int(custom.Samples*MaxSamples + 1.0); i++)
    {
        var res = vec2f(16*16,16*16);
        // pixelSize: width of box filter
        var uv = 2.*((((vec2f(id.xy)+0.5)+(custom.pixelSize*(rand4().xy-0.5)))-.5*res)/(.5*res));
        var bud = buddhabrot(uv);
    }
}
