...
#storage atomic_storage_screen array<atomic<i32>>
...
fn Rasterize(@builtin(global_invocation_id) id: uint3) {
    ...
    let vv = dot(vel,vel);
    var col = abs(vel*0.5 + 0.5)*pow(vv, 2.0);
    ...
    for(var i = 0; i < sampleCount; i++)
    {
        ...
        RasterizePointBH(pos + dpos, 1000.0*col);
    }
}
...
fn Sample(pos: int2) -> float3
{
    ...
    var color: float3;
        ...
        color = tanh(custom.Exposure*0.0003*float(screen_size.x)*float3(x,y,z)/(custom.Samples*MaxSamples + 1.0));
...
fn main_image(@builtin(global_invocation_id) id: uint3) 
{
    ...
    if(mouse.click == 0 && custom.AnimatedNoise > 0.5)
    {
