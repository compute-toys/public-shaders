...
#define bZ 2u       //how many history bits, must be 2^
...
#workgroup_count fun 2048 1 1  //rZ*rZ*rZ*bZ/32u threads
...
fn fun(@builtin(global_invocation_id) id3: uint3)
{
    ...
    var i2 = (id+1u) * 32u/bZ;
    var mv = 0u;
    var hs = hash(id + time.frame * rZ*rZ*rZ*bZ/32u);
    for(var i=i1; i<i2; i++)
    {
        var pu = uint3(i % rZ,
                       i / rZ % rZ,
...
fn main_image(@builtin(global_invocation_id) id: uint3)
{
    ...
    for(var z=0u; z<x4; z++)
    {
        ...
        var n = lig*max(1.f-l*custom.A*float(b!=2u),0.f);  
        rif += (lig-n)*float4(uint4(0u,(b>>0u)&1u,(~(b>>1u))&1u,0u)); 
        lig  = n;
        p += ray*l*1.001f;                                     
        var o = abs(p*x3d-1.f);
