...
fn get_index(idx : u32, idy: u32, screen_size: uint2) -> u32 {
    return  idx + idy * u32(SCREEN_WIDTH);
}
...
fn main_image(@builtin(global_invocation_id) id: uint3) {
    ...
    if (time.frame == 0u) {
        for (var s=0u; s<N; s++) {
            ...
            let rand = fract(sin(f32(i + N * s) / f32(SCREEN_WIDTH)) * 43758.5453123) + .5;
    ...
    // let i = id.x + id.y * SCREEN_WIDTH;
    let i = get_index(id.x, id.y, screen_size);
    let xrgb = vec4(states[i][0], states[i][1], states[i][2], states[i][3]) + .5;
