...
fn GenerateInput(i: uint) {
    store.Input[i] = pow(hash12(float2(uint2(i, time.frame / 30))), 9.);
}
...
fn ScanBucketResult(
    ...
    store.Auxiliary[DTid.x] = scan_pass(GI,
        select(SCAN_TYPE(0), store.Result[DTid.x * SCAN_WORKGROUP_SIZE - 1], DTid.x > 0u));
}
...
fn main_image(@builtin(global_invocation_id) id: uint3) {
    ...
    if (id.x < INPUT_SIZE) {
        ...
        let a = store.Auxiliary[id.x / SCAN_WORKGROUP_SIZE];
        if (u < 5. * a) {
            col = float3(.5);
        }
    }
