...
fn shadeFragment(tri: ptr<function, Triangle>, bary: vec3<f32>) -> vec3<f32> {
    let position = (*tri).vertices * bary;
    let normal = normalize((*tri).normals * bary);
    let uv = (*tri).uvs * bary;
    let diffuse = max(0.1, dot(normal, normalize(vec3<f32>(custom.lightX, custom.lightY, custom.lightZ))));
    let shade = textureSampleLevel(channel1, bilinear, uv, 0).rgb * diffuse;
    return pow(shade, vec3<f32>(1.0 / 2.2));
}

fn worldToClipSpace(worldSpacePos: vec3<f32>) -> vec4<f32> {
    let viewSpacePos = worldSpacePos + vec3<f32>(0.0, 0.0, -2.5);
...
fn drawTriangles(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    drawTriangle(&tri, clipSpaceVerts);
}
