...
//f(x,y) = (x+y)*(z-x+z-y)      z=totalPoints-1
...
fn fun(@builtin(global_invocation_id) id3: vec3u)
{
    ...
    for(var i=0; i<ZT; i++)
    {
        ...
        for(var j=0; j<T; j++)
        {
            ...
            var n2 = f32(i*T+j);
            var z1 = f32(Z-1);
            var n = ((n1+n2)*(z1-n1+z1-n2))*.0000000015f;
            //var n = min((n1+n2),(z1-n1+z1-n2))*.00006f;
            v -= d*nrm*(n-l);
        }
        r = (r+T) % Z;
    }
    ...
    textureStore(screen, b, pow(cos(f32(id1)/f32(Z)*3.14f+2.f+vec4f(1,2,3,4))*.8f,vec4f(2)));
